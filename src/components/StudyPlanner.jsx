    ...
    <div className="flex flex-col gap-5 relative">
      ...
      <div className="flex justify-between items-center border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2">
          <CheckSquare className="w-5 h-5 text-[#00C9B0]" />
      ...
      <section className="bg-slate-950/20 border border-slate-850 p-4 rounded-md flex flex-col gap-3">
        <div className="flex justify-between items-start text-xs">
          <div className="flex flex-col gap-0.5">
            <span className="font-bold text-white leading-none">{phase.name}</span>
            <span className="text-[10px] text-gray-500 font-medium">{phase.desc}</span>
          </div>
      ...
      <form onSubmit={handleAddTaskSubmit} className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 text-xs border border-slate-800 bg-slate-950/40 rounded-md text-white focus:ring-2 focus:ring-[#00C9B0] focus:outline-none placeholder-gray-500 font-semibold"
        />
        <button
          type="submit"
          disabled={suggesting}
          className="px-3.5 bg-[#00C9B0] hover:bg-[#00b29c] text-[#060B18] rounded-md text-xs font-bold transition-all shadow-sm flex items-center justify-center cursor-pointer disabled:opacity-50"
        >
      ...
      {suggestedTasks && (
        <div className="p-4 bg-indigo-950/25 border border-indigo-900/40 rounded-md flex flex-col gap-3.5 animate-scale-up">
          <div className="flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-indigo-400" />
            <span className="text-xs font-bold text-white">Gemini AI Plan Suggestion</span>
          </div>
          
          <div className="text-xs text-gray-300 leading-relaxed">
            Instead of "<span className="font-bold text-white">{pendingTaskTitle}</span>", let's add these bite-sized, structured study milestones:
          </div>

          <div className="flex flex-col gap-1.5 pl-2 border-l border-[#00C9B0]/30 font-space text-[11px] text-[#00C9B0] font-medium">
          ...
          <div className="flex gap-2 mt-1">
            <button
              ...
              className="px-3.5 py-1.5 bg-[#00C9B0] hover:bg-[#00b29c] text-[#060B18] text-xs font-bold rounded-md transition-all cursor-pointer"
            >
              Accept AI Breakdown
            </button>
            <button
              onClick={handleKeepOriginalTask}
              className="px-3.5 py-1.5 bg-slate-800 border border-slate-700 hover:bg-slate-750 text-white text-xs font-bold rounded-md transition-all cursor-pointer"
            >
              Keep Original
            </button>
          </div>
      ...
      <div className="flex flex-col gap-2.5 max-h-[280px] overflow-y-auto pr-1">
        {tasks.length > 0 ? (
          tasks.map(task => (
            <div 
              key={task.id} 
              className={`p-3 bg-slate-900/30 border border-slate-850 rounded-md flex justify-between items-center gap-4 hover:border-slate-800 transition-all ${
                task.status === 'Done' ? 'opacity-60 bg-slate-950/20' : ''
              }`}
            >
              <div className="flex items-center gap-2.5 overflow-hidden">
                ...
                <button
                  ...
                  className={`w-4.5 h-4.5 rounded border flex items-center justify-center shrink-0 cursor-pointer ${
                    task.status === 'Done' 
                      ? 'bg-[#00C9B0] border-[#00C9B0] text-slate-950' 
                      : 'border-slate-700 hover:border-slate-500 bg-slate-950/20'
                  }`}
                >
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                <select
                  ...
                  className="px-2 py-0.5 border border-slate-800 bg-slate-950/40 text-[9px] font-bold text-gray-400 rounded focus:ring-1 focus:ring-[#00C9B0] focus:outline-none cursor-pointer"
                >
          ...
          <span className="text-xs text-gray-500 text-center py-6">No study tasks logged yet. Add one above!</span>
        )}
      </div>
